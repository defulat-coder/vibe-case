...
The studio is a 2px black split frame with 24px corners and a 1px ink gutter: a clean-cream editor (28px padding, minimum 360px column) beside a preview softly tinted with 18% mint, capped by a black 50px toolbar. The editor opens with a 40px ZCOOL “生成页面” heading and a 44px clear-result icon button once output exists; clearing the result returns focus to the Prompt editor. The 8-row Prompt textarea (8,000-character cap) and the variable inputs (500-character cap each, two-column grid) use mist fills and light 1px dividers; every variable input references a shared screen-reader-only 500-character limit hint. The `#generation-studio` target is programmatically focusable after hash navigation so keyboard and assistive-technology users land in the work area. Completed HTML and its record id are kept separately from the active request, so a retry can leave the last successful preview and record link available until a new response succeeds.

Prompt help states the 20-character minimum, the 8,000-character cap, and explicitly says the draft is saved for the current browser session; the textarea describes both the requirement and live count, and if session storage is unavailable, the help text swaps to a warning to copy the Prompt before leaving. Reassurance says generation may take tens of seconds and that the reference image is not saved. An oversized or unreadable image produces an inline `role="alert"` error, a selected reference always exposes a 44px 移除 action, and removing it returns focus to the file picker. The primary action is disabled below 20 characters and swaps 生成效果 for 停止生成 while loading. Generation failures map network, malformed-response, and service errors to actionable Chinese copy instead of exposing provider internals; Prompt, variables, and any previous result remain available.

The preview has three authored states: an empty line (“生成结果会显示在这里”), a loading spinner, and the sandboxed iframe (`sandbox="allow-scripts"`, `referrerPolicy="no-referrer"`) whose `srcDoc` is hardened by `secureSrcDoc`—`<base>` and refresh metas are stripped and a CSP meta locks the document to inline styles/scripts and `data:`/`blob:` images with `connect-src 'none'`. The toolbar names the state (预览 / 正在生成 / 生成完成; retry failures use 保留上次结果 or 生成失败) and carries a yellow 打开记录 link to `/generations/[id]` once complete. The iframe's visual language is unconstrained model output and must never be treated as part of Vibe Case's component system.
...
- **Prompt Editor:** the seven-row textarea uses the mist canvas, a 1px light-divider border, 13px corners, 13px padding, and 1.55 line-height, with the same 8,000-character cap as the API. Keep the editable Prompt visually larger than its controls. A dynamic helper below the field states the 20-character minimum (including the remaining count when short), exposes the current count after the minimum is met, and is associated with the textarea through `aria-describedby`, so a disabled run action or length boundary always has an explanation.
...
- **Result:** return content inside a 1px light-divider frame with 14px corners and a mist-canvas body. A minimum 44px ink toolbar names the result and carries the 44px clear action; clearing the result returns focus to that case's Prompt editor. Images render at full card width; structured text uses a scrollable preformatted region capped at 520px with 18px padding and 14px / 1.65 Manrope copy. The compact `role="status"` region announces running/completed state while the potentially long result body stays out of the live region; preserve the existing error component. New results enter over 200ms with the shared emphasized easing.
...
Generation swaps Generate for Stop while loading, keeps the spinner local to the action, announces preview state through the compact toolbar `role="status"`, and provides a visible clear/reset control after content arrives; the generated iframe remains outside the site's live regions.
